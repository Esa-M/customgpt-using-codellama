...
export default function Panel({}) {
    ...
    const options = ["Semantic search", "Normal chat"]
    const [searchActive, setSearchActive] = useState(0)
    const [searchOption, setSearchOption] = useState("semantic_search")
    ...
    const GenerateResponse = async(Query) =>{
        ...
        const res = await axios.post(`http://localhost:8000/${searchOption}`, {request: Query})
        ...
        const ChatSnapShot = {query: Query, response: res.data.response,  codes: matches || [], searchType: searchOption}
  ...
  return (
    <div data-theme="dim" className=" min-h-screen h-full bg-base-200 lg:w-4/5 w-full relative ">
      {/* <div className="navbar bg-base-200 sticky top-0  "><a className="p-3 Title-font lg:text-3xl">Custom GPT</a></div>
   */}
        <div className='p-14  h-40rem  overflow-y-scroll'>
            {Chat.map((context, idx) => {
                ...
                return (<><div  ref={(Chat.length - 1 == idx) ? ChatSnippetRef : null} className={`flex flex-col text-left mt-5 relative ${(Chat.length != 1 && idx != Chat.length-1) ? ` `:``} `} key={idx}>
                      ...
                      <div className='text-green-400 font-semibold'>{(context.searchType == "semantic_search") ? "Semantic Response" : "Normal Response"}</div>
        ...
        <div className='lg:w-full  z-50 absolute bottom-0 bg-base-200  flex flex-col gap-2 justify-center align-center'>
          <div className='lg:w-full  flex flex-row justify-center align-middle gap-5'>
            {options.map((c, idx)=>{
              return (<button key={idx} className={`${searchActive == idx ? "btn border border-green-300 text-green-300": "btn" }`} onClick={()=>{setSearchActive(idx) ; (idx == 0) ? setSearchOption("semantic_search") : setSearchOption("chat")  }}>{c}</button>)
            })}
          </div>
        <div className='lg:w-full p-3 z-50  bg-base-200  flex flex-row gap-2 justify-center align-center'>
